<?php
	...
	if((mysqli_num_rows(mysqli_query($c, "SELECT * FROM `users` WHERE session='$session'")))&&($email=="")){
		...
		header("Location: http://".$_SERVER['SERVER_NAME']."/vfac/admin/settings/");
	}else{
		...
		if($count==1){
			...
			header("Location: http://".$_SERVER['SERVER_NAME']."/vfac/admin/?error=yes&email=".$session_email."&password=".$session_password."");
		}
	}
?>
