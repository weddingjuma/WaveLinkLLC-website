...
function get_background_design_html() {
	return 
	'<div class="row background_design">
		<div class="col-xs-4 col-md-4 background_design_left">
			<div class="background_design_red_box"></div>
		</div>
		<div class="col-xs-4 col-md-4"></div>
		<div class="col-xs-4 col-md-4 background_design_right">
			<div class="background_design_blue_box"></div>
		</div>
	</div>';
}
...
function get_partial_posts_html($c, $query, $setting) {
	...
	while($post = mysqli_fetch_array( $post_results, MYSQL_ASSOC )) { 
		...
		$post_banner_style = ($has_photo ? 'background-image:url(\''.$post['image1'].'\');' : 'height:auto;');
		$post_type_icon_style = (!$has_photo ? 'display:none;' : '');
		$post_header_style = (!$has_photo ? 'position:relative; top:0px;' : '');
		$post_icon = $post_icons[$post['type']];
		$post_url = 'https://'.$_SERVER['SERVER_NAME'].'/post/?id='.$post['id'];
		$html .=
		'<div class="partial_post">
			<div class="partial_post_banner" style="'.$post_banner_style.'">
				<i class="fa fa-'.$post_icon.' partial_post_type_icon" style="'.$post_type_icon_style.'"></i> 
				<div class="partial_post_header" style="'.$post_header_style.'">
			...
			<div class="partial_post_content">
				...
				<div class="partial_post_footer">
					<div class="partial_post_author_image" style="background-image:url(\''.$post['image1'].'\');"></div>
					<div class="partial_post_subtitle" style="float:left;">by<br />'.$post['author'].'</div>
					<div class="partial_post_link" onclick="location.href=\''.$post_url.'\';">view full post&nbsp;<i class="fa fa-arrow-right"></i></div>
				</div>
			</div>
			<div class="fb-comments" data-href="https://'.$_SERVER['SERVER_NAME'].'/blog/?id='.$post['id'].'" data-width="100%" data-numposts="5" data-colorscheme="light"></div>
		</div>';
	}
	return $html;
}
