<?php
	...
	if($id == "") {
		if(mysql_query("insert into users(first_name, last_name, email, phone, business_name, description, url, status, survey_completed) values('$first_name', '$last_name', '$email', '$phone', '$business_name', '$description', '$url', '$lead_status', '$survey_completed')")){
		...
		if(mysql_query("update users set first_name = '$first_name', last_name = '$last_name', email = '$email', phone = '$phone', business_name = '$business_name', description = '$description', url = '$url', status = '$lead_status', survey_completed = '$survey_completed' where id = '$id'")){
			header("Location: index.php");
		}else{ 
			//header("Location: error.php");
			echo '<img src="http://'.$_SERVER['SERVER_NAME'].'/images/error.png" style="height:20px;" />&nbsp;&nbsp;Database problem.<br>Contact wavelinkllc.com administrator.';
