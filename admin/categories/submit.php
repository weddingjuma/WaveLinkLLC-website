<?php
	...
	if($id == "") {
		if(mysql_query("insert into categories(name, value, type, featured, description, url, image) 
		...
		if(mysql_query("update categories set name = '$name', value = '$value', type = '$type', featured = '$featured', description = '$description', url = '$url', image = '$filenames[0]' where id = '$id'")){
			header("Location: index.php");
		}else{ 
			echo '<img src="http://'.$_SERVER['SERVER_NAME'].'/images/error.png" style="height:20px;" />&nbsp;&nbsp;Database problem.<br>Contact wavelinkllc.com administrator.';
		}
