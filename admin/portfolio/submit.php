<?php
	...
	$order_index = $_POST['order_index']; if(!is_numeric($order_index)) { $order_index = 0; }
	...
	foreach($file_array as $file) {
		...
		$target_file = $target_dir . $i . date("YmdHis") . "portfolio." . $extension;
		$uploadOk = 1;
		...
		if ($uploadOk == 0) {
			...
			if (move_uploaded_file($file["tmp_name"], $target_file)) { 
				$filenames[$i] = "/images/application/" . $i . date("YmdHis") . "portfolio." . $extension;
	...
	if($id == "") {
		if(mysql_query("insert into portfolio(title, description, url, order_index, category1, category2, category3, image1, image2, image3, image4, image5, image6, image7, image8) 
						values('$title', '$description', '$url', $order_index, '$category1', '$category2', '$category3', '$filenames[0]', '$filenames[1]', '$filenames[2]', '$filenames[3]', '$filenames[4]', '$filenames[5]', '$filenames[6]', '$filenames[7]')")){
		...
		if(mysql_query("update portfolio set title = '$title', description = '$description', url = '$url', order_index = $order_index, category1 = '$category1', category2 = '$category2', category3 = '$category3', image1 = '$filenames[0]', image2 = '$filenames[1]', image3 = '$filenames[2]', image4 = '$filenames[3]', image5 = '$filenames[4]', image6 = '$filenames[5]', image7 = '$filenames[6]', image8 = '$filenames[7]' where id = '$id'")){
			header("Location: index.php");
		}else{ 
			//header("Location: error.php");
			echo '<img src="http://'.$_SERVER['SERVER_NAME'].'/images/error.png" style="height:20px;" />&nbsp;&nbsp;Database problem.<br>Contact wavelinkllc.com administrator.';
