<?php
	...
	foreach($file_array as $file) {
		...
		$filename = $i . date("YmdHis") . "special." . $extension;
	...
	if($id == "") {
		if(mysqli_query($c, "insert into specials(title, description, photo, segue, url, order_index)  
						values('$title', '$description', '$filenames[0]', '$segue', '$url', '$order_index')")){
		...
		if(mysqli_query($c, "update specials set title = '$title', description = '$description', photo = '$filenames[0]', segue = '$segue', url = '$url', order_index = '$order_index' where id = '$id'")){
			header("Location: index.php");
		}else{ 
