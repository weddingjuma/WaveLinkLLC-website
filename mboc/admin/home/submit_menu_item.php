<?php
	...
	$segue = addslashes($_POST['segue']);
	$url = addslashes($_POST['url']); if($url == "") { $url = "n/a"; }
	$order_index = addslashes($_POST['order_index']);
	$badge = addslashes($_POST['badge']); if($badge == "") { $badge = "0"; }
	...
	foreach($file_array as $file) {
		...
		$filename = $i . date("YmdHis") . "menu_item." . $extension;
	...
	if($id == "") {
		if(mysqli_query($c, "insert into menu_items(page, name, icon, segue, url, order_index, badge)  
						values('$page', '$name', '$icon', '$segue', '$url', '$order_index', '$badge')")){
		...
		if(mysqli_query($c, "update menu_items set page = '$page', name = '$name', icon = '$icon', segue = '$segue', url = '$url', order_index = '$order_index', badge = '$badge' where id = '$id'")){
			header("Location: index.php");
		}else{ 
